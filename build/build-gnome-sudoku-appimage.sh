...
# 1. Build blueprint-compiler (v0.16.0 is stable for GTK 4.8)
...
# 3. Patch Sudoku for Debian 12 (Libadwaita 1.2 / GTK 4.8)
echo "=== Patching Sudoku for Libadwaita 1.2 ==-"
# Relax versions
sed -i "s/gtk4', version: '>= [0-9.]*'/gtk4', version: '>= 4.8.0'/g" "$PROJECT_DIR/meson.build"
sed -i "s/libadwaita-1', version: '>= [0-9.]*'/libadwaita-1', version: '>= 1.2.0'/g" "$PROJECT_DIR/meson.build"
sed -i "s/glib-2.0', version: '>= [0-9.]*'/glib-2.0', version: '>= 2.74.0'/g" "$PROJECT_DIR/meson.build"
...
foreach my $f (@funcs) {
    while ($content =~ m/\bvoid\s+$f\s*\([^\)]*\)\s*\{/g) {
        ...
        substr($content, $start, $end - $start) = "void stub_$f() { } /* stub */";
    }
}
print $content;
EOF
...
# Blueprint Sophisticated Patcher
...
# 1. Fix SudokuMenuButton spacing
$content =~ s/(\$SudokuMenuButton\s+menu_button)\s*\{/$1 \{/g;

# 2. Downgrade SwitchRow to ActionRow + Gtk.Switch
...
# 3. Downgrade SpinRow to ActionRow + Gtk.SpinButton
...
while ($content =~ m/\bAdw\.StatusPage(?:\s+[a-zA-Z0-9_]+)?\s*\{/g) {
    ...
    my $replacement = "Gtk.Box { orientation: vertical; \n $title \n $desc \n $inner \n}";
    substr($content, $start, $end - $start) = $replacement;
    pos($content) = $start + length($replacement);
}

# 5. Type and property replacements
...
# 7. Final Semicolon Normalization
$content =~ s/\}\s*;/}/g;
$content =~ s/^\s*;\s*$//mg;
