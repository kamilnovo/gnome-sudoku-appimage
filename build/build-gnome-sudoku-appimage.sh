...
for f in "$PROJECT_DIR"/src/blueprints/*.blp; do
    ...
    # 1. Global Semicolon Fix: Semicolons after blocks are optional/forbidden in many contexts
    # but specifically problematic when we move blocks around.
    sed -i 's/};/}/g' "$f"

    # 2. ToolbarView -> Box
    perl -0777 -pi -e 's/Adw.ToolbarView\s*\{/Gtk.Box { orientation: vertical; /g' "$f"
    
    # 3. StatusPage Fix (Specifically for start-view.blp)
    # Convert Adw.StatusPage to a Box + Label + Clamp
    perl -0777 -pi -e 's/Adw.StatusPage\s*\{((?:[^{}]|\{(?1)\})*)\}/
        my $inner = $1;
        my $title = "";
        if ($inner =~ s#title:\s*(_\("[^"]+"\));##) { $title = $1; }
        $inner =~ s#child:##g;
    ...
    # We must preserve the IDs so Vala code can find the widgets.
    ...
    # 7. Dialog types and basic renames
...
# Replace Adw.AlertDialog with Adw.MessageDialog (which has a constructor that works in 1.2)
